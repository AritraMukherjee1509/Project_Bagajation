...
export default function AboutProvider({ provider }) {
  ...
  const handleMessage = () => {
    ...
    navigate(`/messages/${provider._id}`);
  };
  ...
  const getProviderAvatar = () => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(provider?.name || 'Provider')}&background=3b82f6&color=ffffff&size=100`;
  };
        ...
        <div className={s.providerHeader}>
          <div className={s.avatar}>
            <img 
              ...
              onError={(e) => {
                e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(provider.name)}&background=3b82f6&color=ffffff&size=100`;
              }}
            />
          ...
          <div className={s.providerInfo}>
            ...
            <p className={s.providerTitle}>
              {provider.businessInfo?.businessName || 'Professional Service Provider'}
            </p>
      ...
      <div className={s.aboutSection}>
        ...
        <div className={s.bio}>
          <p>
            {provider.experience?.description ? (
              ...
              `${provider.name} is a professional service provider with ${getExperienceText().toLowerCase()}. 
               Known for quality work and excellent customer service.`
            )}
          </p>
          {provider.experience?.description && provider.experience.description.length > 150 && (
            <button 
        ...
        <div className={s.details}>
          <div className={s.detail}>
            ...
            <span>Member since {formatJoinDate(provider.joinedAt)}</span>
          </div>
            ...
            <div className={s.detail}>
              ...
              <span>
                {provider.businessInfo.businessType.charAt(0).toUpperCase() + 
                 provider.businessInfo.businessType.slice(1)} Business
              </span>
            </div>
          )}
